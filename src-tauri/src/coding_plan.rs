// Adapted from cc-switch (MIT), commit 3217f72596f2d1c0f879f0a05f83803825d9809f.
...
async fn query_kimi(api_key: &str) -> Result<SubscriptionQuota, String> {
    ...
    parse_kimi_body(&body)
}

fn parse_kimi_body(body: &serde_json::Value) -> Result<SubscriptionQuota, String> {
...
async fn query_zenmux(base_url: &str, api_key: &str) -> Result<SubscriptionQuota, String> {
    ...
    parse_zenmux_body(&body)
}

fn parse_zenmux_body(body: &serde_json::Value) -> Result<SubscriptionQuota, String> {
    // 检查业务级别错误
...
mod tests {
    ...
    use crate::subscription::CredentialStatus;
    ...
    #[test]
    fn kimi_remaining_and_string_numbers_are_used_percentages() {
        let quota = super::parse_kimi_body(&json!({
            "limits": [{"detail": {"limit": "100", "remaining": "75", "resetTime": 1780329600}}],
            "usage": {"limit": 200, "remaining": 90, "resetTime": "2026-09-01T00:00:00Z"}
        }))
        .unwrap();
        assert_eq!(quota.tiers.len(), 2);
        assert_eq!(quota.tiers[0].utilization, 25.0);
        assert!((quota.tiers[1].utilization - 55.0).abs() < 1e-9);
        assert!(quota.tiers[0]
            .resets_at
            .as_ref()
            .unwrap()
            .starts_with("2026-"));
    ...
    #[test]
    fn kimi_zero_limits_do_not_divide_by_zero() {
        let quota =
            super::parse_kimi_body(&json!({"usage": {"limit": 0, "remaining": 0}})).unwrap();
        assert_eq!(quota.tiers.len(), 1);
        assert_eq!(quota.tiers[0].utilization, 0.0);
    }

    #[test]
    fn zenmux_fractional_usage_and_usd_fields() {
        let quota = super::parse_zenmux_body(&json!({"success": true, "data": {
            "quota_5_hour": {"usage_percentage": "0.25", "used_value_usd": "2.5", "max_value_usd": 10},
            "quota_7_day": {"usage_percentage": 0.5},
            "plan": {"tier": "pro"}, "account_status": "active"
        }})).unwrap();
        assert_eq!(quota.tiers.len(), 2);
        assert_eq!(quota.tiers[0].utilization, 25.0);
        assert_eq!(quota.tiers[0].used_value_usd, Some(2.5));
        assert_eq!(quota.tiers[0].max_value_usd, Some(10.0));
        assert_eq!(quota.tiers[1].utilization, 50.0);
        assert_eq!(quota.credential_message.as_deref(), Some("pro (active)"));
    }

    #[test]
    fn zenmux_business_errors_are_not_zero_usage() {
        let quota =
            super::parse_zenmux_body(&json!({"success": false, "message": "not subscribed"}))
                .unwrap();
        assert!(!quota.success);
        assert!(quota.tiers.is_empty());
        assert!(quota.error.unwrap().contains("not subscribed"));
    }
}
