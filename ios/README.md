...
0.8.0 起，主界面、套餐设置、云同步、定时预热、小组件与灵动岛均使用中文分步说明。首页提供“刷新用量、添加套餐、同步设置”三个明确入口；首次使用会显示三步引导。上传、下载和停用同步前会说明影响并要求确认，减少误覆盖配置。

所有主要操作保留文字标签和 VoiceOver 提示，状态不只依赖颜色表达。动态字体下表单可自然扩展，主要按钮的最小点击高度为 44pt。
...
- 查看、新增、编辑、删除八类套餐配置。
- 查询火山方舟、Kimi、智谱个人/团队、MiniMax、ZenMux、OpenCode Go 的套餐窗口，以及 DeepSeek 余额。
- 应用前台每 60 秒刷新，支持下拉和手动刷新；后台使用 `BGAppRefresh`，实际执行时间由 iOS 决定。
...
- App Group 共享快照包含套餐名称、供应商、用量窗口、重置时间和余额，不包含套餐凭据。
