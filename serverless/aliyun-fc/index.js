...
exports.prewarm = async function prewarm() {
  try {
    ...
    await sendNotifications('TokenPlan：阿里云 FC 预热成功', true);
    console.log(JSON.stringify({ provider: 'aliyun-fc', status: response.status }));
    return { success: true, status: response.status };
  } catch (error) {
    await sendNotifications(`TokenPlan：阿里云 FC 预热失败\n${String(error).slice(0, 300)}`, false);
    throw error;
  }
};
